...
KJ_TAG = "kj-card"
# A machine summary. Tagged apart from cards so that the importer can refuse
# it, and so that a glance in Zotero tells the researcher who wrote it.
SUMMARY_TAG = "kj-summary"
...
def summary_note_html(
    ...
    *,
    ...
    """One machine summary as note HTML.

    The first line says what this is and who wrote it, because in Zotero this
    note sits among notes the researcher wrote, and nothing else in the item
    pane distinguishes them. Paragraphs are the model's own; the marker block
    at the end is what the importer reads to know to leave it alone.
    """
    parts = [
        "<h2>Machine summary — not your reading</h2>",
        "<p><em>Generated by "
        f"{escape_html(summary.get('model') or 'a language model')}"
        " from the attached file. It has not been checked against the text, "
        "and it is not evidence. Nothing here becomes a card.</em></p>",
    ]
    if attachment_title:
        parts.append(f"<p><strong>Of:</strong> {escape_html(attachment_title)}</p>")
    if citation:
        parts.append(f"<p><strong>Source:</strong> {escape_html(citation)}</p>")

    for block in (summary.get("text") or "").split("\n\n"):
        block = block.strip()
        if block:
    ...
    if summary.get("truncated"):
        parts.append(
            "<p><em>Only part of the file was sent — the summary covers that "
            "part and no more.</em></p>"
        )

    parts.append("<hr/>")
    parts.append(
        "<p>"
        f"kj:summary={escape_html(summary['id'])} "
        f"kj:generated={escape_html(summary.get('created_at') or '')} "
        f"kj:model={escape_html(summary.get('model') or '')} "
        f"kj:project={escape_html(project_name)}"
        "</p>"
    )
    return "".join(parts)
...
def summary_note_payload(
    ...
    *,
    ...
    """A summary as a child note of the source item.

    A child note cannot belong to a collection, which is exactly right here:
    the summary must stay out of ``_KJ/Inbox``, where the grouping happens.
    """
    return {
        "itemType": "note",
        "parentItem": parent_item_key,
        "note": summary_note_html(
            summary,
            project_name=project_name,
            citation=citation,
